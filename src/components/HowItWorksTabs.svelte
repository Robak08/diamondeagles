...
<Tabs.Root value="collect" orientation="horizontal">
	<Tabs.List
		class="relative grid items-start gap-6 gap-y-12 lg:gap-y-6 lg:grid-cols-5 h-fit w-full bg-transparent"
	>
		<div
			class="absolute top-[38px] right-0 left-8 -z-10 hidden h-px bg-input lg:block w-[calc(100%_-_3rem)]"
		></div>
		{#each steps as { value, title, description, image }, index}
			<Tabs.Trigger
				key={value + index}
				{value}
				class="group relative pointer-events-none lg:pointer-events-auto h-full hover:bg-muted !border-transparent data-[state=active]:!bg-transparent lg:data-[state=active]:!bg-muted"
			>
				<div
					class="w-full h-full flex flex-col gap-4 lg:block rounded-md p-0 text-left sm:py-4 lg:px-4 text-foreground lg:text-current"
				>
					<!-- horizontal line for lg+ -->
					<div
						class="absolute top-[34px] right-0 left-8 z-1 hidden h-px bg-primary lg:group-data-[state=active]:block w-[calc(100%_-_2rem)]"
					></div>
					<!-- vertical line for lg- -->
					<div
						class="absolute top-8 left-[38px] z-1 h-1/2 w-px bg-muted lg:hidden"
					></div>
					<div
						class="flex items-start gap-3 lg:items-center lg:contents bg-transparent p-4 lg:p-0"
					>
						<!-- NUMBER -->
						<span
							class="flex size-7 relative z-1 shrink-0 items-center justify-center rounded-full border bg-muted lg:bg-background font-mono text-xs font-medium lg:group-data-[state=active]:bg-gradient lg:group-data-[state=active]:text-background lg:group-data-[state=active]:font-semibold lg:group-data-[state=active]:ring lg:group-data-[state=active]:ring-muted-foreground/40"
						>
							{index + 1}
						</span>
						<!-- <span class="h-full w-px bg-input lg:hidden"></span> -->
						<div>
							<h3 class="mb-1 font-medium lg:mt-4 text-base">
								{title}
							</h3>
							<p class="!text-xs xl:!text-sm mt-2 mb-0 text-wrap">
								{description}
							</p>
						</div>
					</div>
					<div class="mt-3 lg:mt-6 block border bg-muted px-4 py-6 lg:hidden">
						<div class="aspect-video">
							<!--<img
									...
									/> -->
							<div class="placeholder aspect-3/2">IMAGE PLACEHOLDER</div>
						</div>
					</div>
				</div>
			</Tabs.Trigger>

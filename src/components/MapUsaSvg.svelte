...
<style lang="postcss">
	...
	svg {
		@apply opacity-25 dark:opacity-50 pointer-events-none;
        @apply stroke-1 stroke-primary/50 dark:stroke-primary/25;
		@apply translate-y-1/2 lg:translate-y-1/5 lg:scale-[1.2];
		.state {
			@apply fill-primary/20 dark:fill-primary/10;
		} /* default color for all states not specified below */
		.borders {
			@apply stroke-none;
		} /* color and width of borders between states */
		.separator1 {
			@apply stroke-1 stroke-gray-200 dark:stroke-gray-600;
		}
		.state {
		}
	}
